...
exports.exportItemsCSV = async (req, res, next) => {
  try {
    console.log("Export CSV request received:", req.query);
    ...
    if (itemIds) {
      try {
        ...
        // Use string IDs directly without conversion
        query._id = { $in: itemIdArray };
        console.log("Using item IDs:", itemIdArray);
      } catch (error) {
...
exports.exportItemsJSON = async (req, res, next) => {
  try {
    console.log("Export JSON request received:", req.query);
    const { folderId, tags, lowStock, itemIds } = req.query;
